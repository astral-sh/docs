...
PyTorch doesn't publish ROCm builds for macOS or Windows. As such, we gate on `sys_platform` to instruct uv to limit the PyTorch index to Linux, falling back to PyPI on macOS and Windows:
...
Since GPU-accelerated builds aren't available on macOS, the above configuration will fail to install on macOS when the `cu130` extra is enabled.

## [Installing GPU-enabled PyTorch extensions](#installing-gpu-enabled-pytorch-extensions)
