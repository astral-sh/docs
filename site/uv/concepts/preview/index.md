...
- `build-dependency-check`: Checks build dependencies before nonisolated builds with `uv build`.
...
- `lock-without-metadata`: Omit `package.metadata` from `uv.lock`, except for remote URL dependencies.
