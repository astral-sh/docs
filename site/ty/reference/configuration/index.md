...
### [`allowed-unresolved-imports`](#allowed-unresolved-imports)
...
### [`strict-literal-narrowing`](#strict-literal-narrowing)

Whether equality-based checks should preserve broad builtin types rather than narrow them to literal types.

By default, ty narrows `value` from `str` to `Literal["a"]` in the positive branch of `value == "a"`. When this option is enabled, `value` remains `str`. This also applies to membership tests and literal match patterns, which use equality comparisons.
...
Broad builtin types include subclasses, but literal types distinguish values by both their runtime type and value. This makes the narrowing unsound even for subclasses that inherit builtin equality. For example:
...
The standard library's `StrEnum` and `IntEnum` types are also subclasses of `str` and `int`, respectively. This means enum members can encounter the same unsoundness:
...
Specifies the target platform that will be used to analyze the source code. If specified, ty will understand conditions based on comparisons with `sys.platform`, such as are commonly found in typeshed to reflect the differing contents of the standard library across platforms. If `all` is specified, ty will assume that the source code can run on any platform.
...
An override allows you to apply different rule configurations to specific files or directories. Multiple overrides can match the same file, with later overrides take precedence. Override rules take precedence over global rules for matching files.
...
## [`overrides.analysis`](#overridesanalysis)

#### [`allowed-unresolved-imports`](#allowed-unresolved-imports_1)
...
#### [`respect-type-ignore-comments`](#respect-type-ignore-comments_1)
...
#### [`strict-literal-narrowing`](#strict-literal-narrowing_1)
...
If left unspecified, ty will try to detect common project layouts and initialize `src.root` accordingly. The project root (`.`) is always included. Additionally, the following directories are included if they exist and are not packages (i.e. they do not contain `__init__.py` or `__init__.pyi` files):
