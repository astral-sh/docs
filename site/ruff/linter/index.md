# [The Ruff Linter](#the-ruff-linter)
...
The set of enabled rules is controlled via the [`lint.select`](../settings/#lint_select), [`lint.extend-select`](../settings/#lint_extend-select), and [`lint.ignore`](../settings/#lint_ignore) settings.
...
Rule selectors like [`lint.select`](../settings/#lint_select) and [`lint.ignore`](../settings/#lint_ignore) accept either a full rule code (e.g., `F401`) or any valid prefix (e.g., `F`). For example, given the following configuration file:
...
Ruff would enable all rules with the `E` (pycodestyle) or `F` (Pyflakes) prefix, with the exception of `F401`. For more on configuring Ruff via `pyproject.toml`, see [*Configuring Ruff*](../configuration/).
...
- Prefer [`lint.select`](../settings/#lint_select) over [`lint.extend-select`](../settings/#lint_extend-select) to make your rule set explicit.
...
To resolve the enabled rule set, Ruff may need to reconcile [`lint.select`](../settings/#lint_select) and [`lint.ignore`](../settings/#lint_ignore) from a variety of sources, including the current `pyproject.toml`, any inherited `pyproject.toml` files, and the CLI (e.g., [`--select`](../settings/#lint_select)).

In those scenarios, Ruff uses the "highest-priority" [`select`](../settings/#lint_select) as the basis for the rule set, and then applies [`extend-select`](../settings/#lint_extend-select) and [`ignore`](../settings/#lint_ignore) adjustments. CLI options are given higher priority than `pyproject.toml` options, and the current `pyproject.toml` file is given higher priority than any inherited `pyproject.toml` files.
...
When [preview mode](../preview/) is enabled, rule selectors also accept the human-readable name of a rule (e.g., `unused-import`).

## [Rule categories](#rule-categories)

In [preview](../preview/), Ruff supports rule categories in addition to the Flake8-style linter groups described above. These categories organize rules by the types of issues they detect and determine whether rules are enabled by default. These categories and their descriptions, in order of decreasing severity, are:

- **Correctness**: These rules flag code that is outright wrong as written. If you encounter a correctness issue, you should try to fix it rather than suppressing the error with `noqa` or `ruff: ignore`.
- **Suspicious**: These rules are similar to `correctness` lints in that the code is likely wrong, but `suspicious` lints acknowledge that there are valid reasons for the code to be written in this way. You will still typically want to fix these issues, but using a suppression comment may occasionally be necessary. Deprecations generally also fit into this category.
- **Complexity**: These rules detect code that can be written in a simpler or more readable way without changing its semantics.
...
- **Style**: These rules flag code that could be written more idiomatically and where the relevant idiom has broad community acceptance.
...
will select all `E` and `F` rules, with the exception of `F401`. Analogously, a selection with the `suspicious` category like:
...
Note that we plan to deprecate and eventually remove the linter groups in the future. If you give the new categories a try and run into situations where you need to fall back on linter groups, please let us know on the [tracking issue](https://github.com/astral-sh/ruff/issues/27959).
...
By default, Ruff will fix all violations for which safe fixes are available; to determine whether a rule supports fixing, see [*Rules*](../rules/).
...
Ruff only enables safe fixes by default. Unsafe fixes can be enabled by settings [`unsafe-fixes`](../settings/#unsafe-fixes) in your configuration file or passing the `--unsafe-fixes` flag to `ruff check`:
...
By default, Ruff will display a hint when unsafe fixes are available but not enabled. The suggestion can be silenced by setting the [`unsafe-fixes`](../settings/#unsafe-fixes) setting to `false` or using the `--no-unsafe-fixes` flag.

The safety of fixes can be adjusted per rule using the [`lint.extend-safe-fixes`](../settings/#lint_extend-safe-fixes) and [`lint.extend-unsafe-fixes`](../settings/#lint_extend-unsafe-fixes) settings.
...
To limit the set of rules that Ruff should fix, use the [`lint.fixable`](../settings/#lint_fixable) or [`lint.extend-fixable`](../settings/#lint_extend-fixable), and [`lint.unfixable`](../settings/#lint_unfixable) settings.

For example, the following configuration would enable fixes for all rules except [`unused-imports`](../rules/unused-import/) (`F401`):
...
To omit a lint rule everywhere, add it to the "ignore" list via the [`lint.ignore`](../settings/#lint_ignore) setting, either on the command-line or in your `pyproject.toml` or `ruff.toml` file.

To omit a lint rule within specific files based on file path prefixes or patterns, see the [`lint.per-file-ignores`](../settings/#lint_per-file-ignores) setting.
...
In [`preview`](../preview/) mode, rule names (e.g. `unused-import`) can be used in `ruff: ignore`, `ruff: file-ignore`, `ruff: disable`, and `ruff: enable` comments instead of rule codes (e.g. `F401`).

#### [Line-level](#line-level)

Ruff supports a `noqa` system similar to [Flake8](https://flake8.pycqa.org/en/3.1.1/user/ignoring-errors.html). To ignore an individual violation, add `# noqa: {code}` to the end of the line, like so:
...
Ruff can also *remove* any unused suppression comments via its fix functionality. To remove any unused suppressions, run Ruff with `--fix`, like so:

```
$ ruff check /path/to/file.py --extend-select RUF100 --fix
```

### [Inserting necessary suppression comments](#inserting-necessary-suppression-comments)
...
See the [isort documentation](https://pycqa.github.io/isort/docs/configuration/action_comments.html) for more.
