...
If a path including the leading `@` exists, Ruff treats it as a literal path instead. For example, `ruff check @list.py` checks `@list.py` if it exists, even if `list.py` also exists. Otherwise, Ruff reads arguments from `list.py`. This applies to files and directories, including arguments after `--` and arguments inside response files.
