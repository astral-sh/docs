...
### [Trailing end-of-line comments](#trailing-end-of-line-comments)
...
This is similar to [Pyink](https://github.com/google/pyink) but a deviation from Black. Black avoids splitting any lines that contain a `# type` comment ([#997](https://github.com/psf/black/issues/997)), but otherwise avoids special-casing pragma comments.
...
### [Line width vs. line length](#line-width-vs-line-length)
...
### [Parenthesizing long nested-expressions](#parenthesizing-long-nested-expressions)
...
### [Call expressions with a single multiline string argument](#call-expressions-with-a-single-multiline-string-argument)
...
### [Blank lines at the start of a block](#blank-lines-at-the-start-of-a-block)
...
For more details on the formatting style, refer to the [f-string formatting](../#f-string-formatting) section.

### [Implicit concatenated strings](#implicit-concatenated-strings)
...
### [`global` and `nonlocal` names are broken across multiple lines by continuations](#global-and-nonlocal-names-are-broken-across-multiple-lines-by-continuations)
...
### [Trailing own-line comments on imports are not moved to the next line](#trailing-own-line-comments-on-imports-are-not-moved-to-the-next-line)
...
### [Parentheses around awaited collections are not preserved](#parentheses-around-awaited-collections-are-not-preserved)
...
### [Implicit string concatenations in attribute accesses](#implicit-string-concatenations-in-attribute-accesses)
...
### [Own-line comments on expressions don't cause the expression to expand](#own-line-comments-on-expressions-dont-cause-the-expression-to-expand)
...
### [Tuples are parenthesized when expanded](#tuples-are-parenthesized-when-expanded)
...
### [Single-element tuples are always parenthesized](#single-element-tuples-are-always-parenthesized)
...
Adding parentheses around single-element tuples adds visual distinction and helps avoid "accidental" tuples created by extraneous trailing commas (see, e.g., [#17181](https://github.com/django/django/pull/17181)).

### [Parentheses around call-chain assignment values are not preserved](#parentheses-around-call-chain-assignment-values-are-not-preserved)
...
### [Call chain calls break differently in some cases](#call-chain-calls-break-differently-in-some-cases)
...
### [Single `with` item targeting Python 3.8 or older](#single-with-item-targeting-python-38-or-older)
...
### [The last context manager in a `with` statement may be collapsed onto a single line](#the-last-context-manager-in-a-with-statement-may-be-collapsed-onto-a-single-line)
...
### [Preserving parentheses around single-element lists](#preserving-parentheses-around-single-element-lists)

Ruff preserves at least one set of parentheses around list elements, even if the list only contains a single element. The Black 2025 style or newer, on the other hand, removes the parentheses for single-element lists if they aren't multiline and doing so does not change semantics:
...
### [Long lambda expressions](#long-lambda-expressions)

Both Ruff and Black will keep lambda parameters on a single line. However, if the body expression exceeds the configured line length, Ruff will additionally add parentheses around the lambda body and break it over multiple lines:
...
### [Require single blank line between function and decorated class in stubs](#require-single-blank-line-between-function-and-decorated-class-in-stubs)

In stub files, Ruff enforces a single blank line between functions and decorated classes, whereas Black does not.
...
### [Avoiding additional space around escaped quote in triple-quoted docstring](#avoiding-additional-space-around-escaped-quote-in-triple-quoted-docstring)

In certain cases it is required to maintain a trailing space in a triple-quoted docstring in order to avoid escaping a quote or ending with too many quotes. However, if the last quote is escaped this is not necessary, and Ruff respects this. Black, however, adds the extra space.
